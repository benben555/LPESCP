...
def KeywordEnc(SPM, kw, IDA, IDB):
    ...
    while gy.gcd(n, r) != 1:
        r += 1
    s=random.randint(1,1001)
    c1=user[IDB].PK_U3
    C5= gy.powmod(c1, s, n ** 2)*gy.powmod(g,s*int(KC_KW),n**2)*gy.powmod(r,n,n**2) % (n**2)
...
def TrapdoorGen(SPM, kw, IDA, IDB):
    ...
    s=random.randint(1,1001)
    c1=user[IDA].PK_U3
    TD3= gy.powmod(c1, s, n ** 2)*gy.powmod(g,s*int(KC_KW),n**2)*gy.powmod(r,n,n**2) % (n**2)
    return TD1, TD2, TD3


def MatchTest(SPM, C1, C2, C3, C4,C5 ,TD1, TD2,TD3,IDU):
    print(C5)
    F=user[IDA].Pai.decipher(C5)
    if int.from_bytes(F, byteorder='big', signed=False)<(int(user[IDU].N_U)//2):
        print("fast")
    else:
        print("slow")
    print("MatchTest:")
...
if __name__ == "__main__":
    ...
    kw1="urgent"
    TD1, TD2, TD3 = TrapdoorGen(SPM, kw1, IDB, IDC)
    if MatchTest(SPM, C1, C2, C3, C4,C5, TD1, TD2,TD3,IDB):
        print("转发解密")
